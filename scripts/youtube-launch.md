...
Companion to `youtube-metadata.md`. The titles, descriptions and tags below are
the same ones, assembled ready to paste: the site link is already on the first
line, the footer slugs are filled in, and each paragraph is a single line so
YouTube reflows it on phones instead of double-wrapping the markdown's hard
wraps.

Videos are in `video/`, which is gitignored. Six files, 4.1 GB total.

## 1. Create the channel

- [ ] At <https://www.youtube.com/channel_switcher>, click **Create a channel**.
      This makes a Brand Account: transferable, multi-manager, not welded to your
      name. Your existing channel is untouched and none of its subscriptions or
      history carry over.
...
- [ ] Switch to the Colored Noise channel (avatar → **Switch account**).
      Verification does not carry over from another channel.
- [ ] Verify at <https://www.youtube.com/verify> by SMS or automated call.
...
- [ ] Confirm Studio → Settings → Channel → **Feature eligibility** shows
      "Features that require phone verification: **Enabled**". Usually instant,
      occasionally up to 24 hours. Do not start uploading until it does.

## 3. Brand it

- [ ] **Profile picture** — largest square in `icons/`. Minimum 98x98, under 4 MB.
- [ ] **Banner** — `video/channel-banner.png`, regenerate with
...
Run `caffeinate -d -i -m` in a terminal tab first, or the Mac sleeps mid-upload.
Upload one file at a time — the browser uploader resumes, but not reliably
enough to test on six 640 MB files at once.
...
- [ ] **Check the Restrictions column** for Content ID claims. Automated claims
      on generic noise are common and usually released without escalation.
